...
class cursos_alumno extends f
{
    ...
    function cursos_alumno()
    {
        ...
        $this->html        = $this->load()->lib("html_cursos_alumno");
        $this->modelo      = $this->load()->model("modelo");
        $this->baseurl     = BASEURL;
        $this->modelo2     = $this->load()->model("MySQLiManager");
        $this->modelo3     = $this->load()->model("Monodon");
        $this->id        = $this->uri()->seg(2);
    }
    function index()
    {
        if ($this->valida(5)) {
            $h["title"]   = "Detalle Matrícula";
            $c["content"] = $this->html->container($this->id);
            $c["title"]   = "Detalle Matrícula";
            $this->View($h, $c);
    ...
    {
        //$alumnos = json_decode($this->modelo3->select_one("usuarios", array("id" => $_GET['id_alumno'])));

        $data = json_decode($this->modelo3->select_all_where("comentarios", array(
    ...
    public function loadcursos()
    {
        ...
        $data = json_decode($this->modelo3->select_all_where("cursos", array("id_grado" => $alumnos->id_grado)));
        //echo json_encode($data->Records);
        $values = array();
        foreach ($data->Records as $key) {
            $sql = "SELECT AVG(n.porcentaje*(nn.nota/100)) promedio FROM notas as n LEFT JOIN tbl_notas nn ON nn.id_nota = n.id AND nn.id_alumno = " . $_GET['id_alumno'] . " AND nn.id_curso = " . $key->id;
            $notas = json_decode($this->modelo3->run_query($sql, false));
            $key->notas = $notas;
            $values[] = $key;
        }
